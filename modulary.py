...
_MODULES_PATH = os.path.abspath("__modulary__")
    ...
    def _get_module(self, module_info):
        if module_info.type == _TYPE_GIT:
            ...
            try:
                ...
                if os.path.exists(os.path.join(module_info.path, 'requirements.txt')):
                    print(os.path.join(module_info.path, 'requirements.txt'))
                    args = ["install", "-r", os.path.join(module_info.path, 'requirements.txt'), "--isolated", "--quiet"]
                    pip_main(args)
